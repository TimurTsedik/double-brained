...
class MeResponse(BaseModel):
    """Кем распознан вызывающий: проба цепочки «токен → пространство → данные»."""
...
def create_v1_router(provider: ApiRuntimeProvider | None = None) -> APIRouter:
    ...
    async def read_me(
        ...
        Ровно проба цепочки «токен → пространство → данные»: единственный
        эндпоинт, которым проверяется, что вход работает целиком. Всё
        содержательное — следующие слайсы.
        """
