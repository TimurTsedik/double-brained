"""Презентация результата обработки апдейта в отправки Telegram-гейтвея.

Единственная точка, где ``AcknowledgementKind`` превращается в конкретные
вызовы гейтвея (панель, подборы, сводка, подтверждения, generic-ack).
Переиспользуется двумя потребителями с одинаковым поведением байт-в-байт:
поллером (long-polling) и inbox-шагом воркера (webhook-путь). Ветки перенесены
из поллера без изменений: каждая обязательная отправка повторяется до успеха
(бэкофф 1с), generic-ack — best-effort.
"""
...
class TelegramPresenter:
    """Отправляет пользователю ответ, соответствующий результату обработки."""
    ...
    async def present(self, update: TelegramUpdate, result: UpdateResult) -> None:
        """Все ветки «result.kind → гейтвей» одного апдейта, порядок как в поллере."""
        ...
        if result.kind is AcknowledgementKind.SEARCH_COMPLETED and getattr(
            ...
            if search_panel is None:
                raise RuntimeError("fresh search action did not return a search panel")
            while True:
                try:
